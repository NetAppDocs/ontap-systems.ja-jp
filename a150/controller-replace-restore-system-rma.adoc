...
summary: システムを完全に動作可能な状態に復元するには、NetApp Storage Encryption構成を復元し、新しいコントローラのライセンスをインストールし、キットに同梱されているRMA手順書に記載されているとおりに故障した部品をNetAppに返却する必要があります。 
---
= AFF A150システムのコントローラーの交換を完了する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A150システムのコントローラー交換を完了するには、NetApp Storage Encryption構成を復元し（必要な場合）、論理インターフェイス（LIF）がホームポートにレポートしていることを確認し、クラスタの健全性チェックを実行して、障害が発生した部品をNetAppに返却します。
