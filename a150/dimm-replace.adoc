...
summary: ストレージシステムがヘルスモニターアラートに基づく過剰なCECCエラーや訂正不能なECCエラーなどのエラーに遭遇した場合、コントローラー内のDIMMを交換する必要があります。これらのエラーは通常、単一のDIMMの故障によって発生し、ストレージシステムがONTAPを起動できなくなります。 
---
= AFF A150 システムの DIMM を交換してメモリエラーを修正する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ストレージシステムが過剰な訂正可能（CECC）または訂正不可能なメモリエラーを検出した場合は、AFF A150システムのDIMMを交換してください。このようなエラーにより、ストレージシステムがONTAPを起動できなくなる場合があります。
