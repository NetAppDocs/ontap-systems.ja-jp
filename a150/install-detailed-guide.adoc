...
---
= 詳細な手順書を使用してAFF A150システムをインストールする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A150ストレージシステムのインストールは、サイトの準備からONTAPクラスタの起動まで、これらの詳細なステップバイステップの手順に従って行います。

MetroCluster 構成の場合は、を使用します https://docs.netapp.com/us-en/ontap-metrocluster/index.html["MetroCluster のドキュメント"^]。

*警告：* ONTAP 9.13.1P8 - 9.13.1P11、ONTAP 9.14.1P1 - 9.14.1P7、または ONTAP 9.15.1 - 9.15.1P2 をインストールする場合で、システムに 10 台以上の内蔵ソリッドステートドライブが含まれている場合は、インストールに向けてシステムを準備するための追加の手順を実行してください。問題  https://mysupport.netapp.com/site/bugs-online/product/ONTAP/JiraNgage/CONTAP-285173["ONTAP-285173-ADPが10本以上の内蔵ドライブを搭載したAFF A150にスペアルートパーティションを残さない"^] を参照してください。
...
** ストレージシステム用のラックスペース
** No.2 プラスドライバ
** システムをネットワーク スイッチおよびウェブブラウザを搭載したノートパソコンまたはコンソールに接続するための追加のネットワークケーブル
** RJ-45接続端子とウェブブラウザへのアクセス機能を備えたノートパソコンまたはコンソール
...
システムを4ポストラックまたは NetApp システムキャビネットに設置します（該当する場合）。
...
--
イーサネットネットワーク構成:: 次の図に示すように、Cat 6 RJ45ケーブルを使用して、e0c～e0fポートをホストネットワークに接続します。
+
--
...
+
.. エクスプローラを開きます。
.. 左側のペインで「ネットワーク」を選択し、右クリックして「更新」を選択します。
