...
summary: ストレージリソースの制御を交換用コントローラに戻して、AFF A1Kシステムが通常の運用を再開できるようにします。 
---
= AFF A1K システムのコントローラーを返却して通常運用を再開する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ストレージリソースの制御を交換用コントローラに戻して、AFF A1Kシステムが通常の運用を再開できるようにします。返却手順は、システムで使用されている暗号化の種類（暗号化なしまたはオンボード キー マネージャー (OKM) 暗号化）によって異なります。
...
+
.. クラスターのキー管理バックアップ情報を表示するには、次のコマンドを実行します。
+
`security key-manager onboard show-backup`

.. オンボードキーマネージャのバックアップを確認するには、次のコマンドを実行します。
+
`security key-manager onboard verify-backup`



. OKMのパスフレーズを入力し、検証が正常に完了したことを確認してください。
+
