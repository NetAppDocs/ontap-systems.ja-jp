...
summary: AFF A20、 AFF A30、またはAFF A50 システムに新しいブート メディア デバイスをインストールした後、USB ドライブからリカバリ イメージを手動で起動して、パートナー ノードから構成を復元できます。 
---
= 手動ブートリカバリプロセスを使用して、AFF A20、AFF A30、またはAFF A50システムのリカバリイメージを起動します
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A20、 AFF A30、またはAFF A50 ストレージ システムに新しいブート メディア デバイスをインストールした後、USB ドライブからリカバリ イメージを手動でブートして、パートナー ノードから構成を復元できます。
...
| システム構成 | 作業 
 ...
 a| 
ネットワーク接続が確立されており、ONTAP 9.16.0 以前を実行している
 a| 
...
*** システムが暗号化を使用している場合は、link:bootmedia-encryption-restore.html["アンコウカノ"] 。
 ...
 a| 
ネットワーク接続が確立されており、ONTAP 9.16.1 以降が実行されている
 a| 
...
ネットワーク接続がありません
 a| 
.. プロンプトが表示されたら、 `N`を押してバックアップ構成を復元します。
.. プロンプトが表示されたら、システムをリブートします。
.. 表示されたメニューから「 * Update flash from backup config * （ sync flash ）」オプションを選択します。
+
アップデートを続行するかどうかを尋ねられたら、 `Y`を押してください。

.. ブートメニューが表示されたら、 `Ctrl-C`を押してください。
.. 次のいずれかを実行します。
+
*** システムで暗号化が使用されていない場合は、ブート メニューから [オプション 1 通常ブート] を選択します。
*** システムが暗号化を使用している場合は、link:bootmedia-encryption-restore.html["アンコウカノ"] 。
