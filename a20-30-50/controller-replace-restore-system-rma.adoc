...
[role="lead"]
NetAppストレージ暗号化設定のリストア（必要な場合）、交換用コントローラのライセンスのインストール、論理インターフェイス（LIF）がホームポートに報告されていることの確認、クラスタヘルスチェック、交換用コントローラのシリアル番号の登録、障害が発生したパーツのNetAppへの返却を行い、AFF A20、AFF A30、またはAFF A50のコントローラの交換手順の最後の手順を完了します。
