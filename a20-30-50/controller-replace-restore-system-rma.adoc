...
summary: AFF A20、AFF A30、またはAFF A50ストレージシステムのコントローラ交換を完了するには、NetApp Storage Encryption構成をリストアし、新しいコントローラに必要なライセンスをインストールします。論理インターフェイスがホームポートにレポートしていることを確認し、クラスタヘルスチェックを実行します。新しいコントローラのシリアル番号を登録し、障害が発生した部品をNetAppに返却します。 
---
= AFF A20、AFF A30、またはAFF A50システムのコントローラーの交換を完了する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A20、 AFF A30、またはAFF A50 ストレージ システムのコントローラの交換を完了するには、まずNetApp Storage Encryption 構成を復元し (必要な場合)、新しいコントローラに必要なライセンスをインストールします。次に、論理インターフェイス (LIF) がホーム ポートにレポートしていることを確認し、クラスターのヘルス チェックを実行します。最後に、新しいコントローラのシリアル番号を登録し、故障した部品をNetAppに返送します。
...
_replacement_ ノードをサービスに戻す前に、LIF がホームポートに接続されていることを確認し、AutoSupport が有効になっている場合は _replacement_ ノードのシリアル番号を登録し、クラスタの健全性を確認し、メンテナンスウィンドウをすべて閉じ、自動ギブバックをリセットする必要があります。
