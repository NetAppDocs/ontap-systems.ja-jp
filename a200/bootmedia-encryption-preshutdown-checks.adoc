...
** コマンドの出力に <lno-DARE> が表示されず、システムで ONTAP 9.5 が実行されている場合は、に進みます  1: Checking NVE or NSE on systems running ONTAP 9.5 and earlier。
** コマンドの出力に <lno-DARE > が表示されず、システムで ONTAP 9.6 以降が実行されている場合は、に進みます  2: Checking NVE or NSE on systems running ONTAP 9.6 and later。
