...
summary: '環境変数を確認したら、オンボード キー マネージャー (OKM)、 NetAppストレージ暗号化 (NSE)、またはNetAppボリューム暗号化 (NVE) が有効になっているシステムに固有の手順を完了する必要があります。AFF A220システムは、手動のブート メディア リカバリ手順のみをサポートします。' 
---
= リストア暗号化- AFF A220
:allow-uri-read: 
:icons: font
...
[role="lead"]
交換用ブート メディアで暗号化を復元します。AFF A220システムは、手動のブート メディア リカバリ手順のみをサポートします。自動ブート メディア リカバリはサポートされていません。
