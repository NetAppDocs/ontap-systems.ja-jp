...
summary: AFF A250 システムの交換用ブートメディアの暗号化をリストアします。オンボードキーマネージャ、NetApp Storage Encryption、または NetApp Volume Encryption が有効になっているシステム固有の手順を実行します。 
---
= AFF A250 システムのブートメディアの暗号化を手動ブートリカバリプロセス後に復元する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A250 システムの交換用ブートメディアで暗号化を復元し、継続的なデータ保護を確保します。交換プロセスには、キーの可用性の確認、暗号化設定の再適用、データへの安全なアクセスの確認が含まれます。
...
====
.次の手順
ブートメディアの暗号化を復元した後、link:bootmedia-complete-rma.html["故障した部品をNetAppに返却します。"]。
