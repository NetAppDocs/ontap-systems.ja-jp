...
+
MetroCluster 構成では、交換後に通常の状態に戻るまで数分かかります。この時点で各ノードの状態が設定済みになります。 DR ミラーリングは有効で、通常モードになります。MetroCluster の node-show -fields node-systemid_command 出力には、 MetroCluster の設定が通常の状態に戻るまで古いシステム ID が表示されます。
