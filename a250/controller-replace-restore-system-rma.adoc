...
summary: AFF A250 システムのコントローラ交換を完了するには、必要に応じて NetApp Storage Encryption 構成をリストアし、新しいコントローラのライセンスをインストールして、障害が発生した部品を NetApp に返却します。 
---
= AFF A250システムのコントローラーの交換を完了する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A250システム コントローラの交換を完了するには、（必要に応じて）NetApp Storage Encryptionの構成を復元し、論理インターフェイス（LIF）がホームポートにレポートしていることを確認し、クラスターのヘルスチェックを実行して、障害が発生したパーツをNetAppに返却します。
