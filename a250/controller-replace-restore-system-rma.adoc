...
[role="lead"]
AFF A250 システムのコントローラ交換を完了して完全に動作させるには、NetApp Storage Encryption 構成をリストア（必要な場合）し、新しいコントローラのライセンスをインストールして、キットに同梱されている RMA 手順書の説明に従って障害が発生した部品を NetApp に返却する必要があります。
