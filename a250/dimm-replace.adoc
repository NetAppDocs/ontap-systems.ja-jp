...
summary: AFF A250 コントローラで、ヘルスモニターアラートに基づく過剰な CECC エラーが発生した場合、またはストレージシステムが ONTAP を起動できない修正不可能な ECC エラーが発生した場合は、DIMM を交換してください。 
---
= AFF A250 システムの DIMM を交換してメモリエラーを修正する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ストレージシステムが過剰な訂正可能（CECC）または訂正不可能なメモリエラーを検出した場合は、AFF A250システムのDIMMを交換してください。このようなエラーにより、ストレージシステムがONTAPを起動できなくなる場合があります。
