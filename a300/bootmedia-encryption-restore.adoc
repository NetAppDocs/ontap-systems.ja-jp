...
summary: 交換用ブートメディアで暗号化をリストアします。Onboard Key Manager、NetApp Storage Encryption、またはNetApp Volume Encryptionが有効になっているシステムに固有の手順を完了します。AFF A300システムは、手動のブートメディアリカバリ手順のみをサポートしています。 
---
= AFF A300システムのブートメディアの暗号化を復元します
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
交換用ブート メディアで暗号化を復元します。AFF A300システムは、手動のブート メディア リカバリ手順のみをサポートします。自動ブート メディア リカバリはサポートされていません。
