...
summary: システムで修正可能なエラー訂正コードまたは修正不可能なECCエラーの数が増加している場合は、AFF A300コントローラモジュールのDIMMを交換してください。故障したDIMMを交換しないと、システムパニックが発生する可能性があります。 
---
= AFF A300 システムの DIMM を交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ストレージシステムが過剰な訂正可能なECCエラーまたは訂正不可能なECCエラー（通常は単一のDIMMの故障によって発生する）を検出した場合は、AFF A300システムのDIMMを交換してください。
