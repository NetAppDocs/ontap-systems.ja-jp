...
summary: システムでフラッシュの寿命がほぼ終了したこと、または特定された NVDIMM が正常でないことが登録された場合は、AFF A320 コントローラモジュールの NVDIMM を交換してください。障害のある NVDIMM を交換しないと、システムパニックが発生する可能性があります。 
---
= AFF A320 システムの NVDIMM を交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A320システムがフラッシュメモリの寿命がほぼ尽きている、または識別されたNVDIMMが正常でないと認識した場合は、NVDIMMを交換してください。故障したNVDIMMを交換しないと、システムパニックを引き起こす可能性があります。
