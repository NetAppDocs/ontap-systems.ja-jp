...
summary: 2ノードMetroCluster構成でFRU交換を完了したら、MetroClusterスイッチバック処理を実行して構成を通常の動作状態に戻します。これにより、障害が発生していたサイトの同期元Storage Virtual Machineがアクティブになり、ローカルディスクプールからデータを提供するようになります。 
---
= AFF A400システムの2ノードMetroCluster構成でアグリゲートをスイッチバックする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
