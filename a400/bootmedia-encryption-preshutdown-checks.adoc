...
[role="lead"]
AFF A400ストレージシステムのデータセキュリティを維持するために、コントローラをシャットダウンする前に、ブートメディアの暗号化キーのサポートとステータスを確認してください。お使いの ONTAP バージョンが NetApp Volume Encryption （NVE）をサポートしているかどうかを確認し、キーマネージャがアクティブであることを確認してください。
