...
summary: 交換用ブートメディアの暗号化をリストアします。オンボードキーマネージャ、NetApp Storage Encryption、またはNetApp Volume Encryptionが有効になっているシステムに固有の手順を実行します。 
---
= AFF A400 システムの手動ブートリカバリプロセス後にブートメディアの暗号化を復元する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A400 システムの交換用ブートメディアで暗号化を復元し、継続的なデータ保護を確保します。交換プロセスには、キーの可用性の確認、暗号化設定の再適用、データへの安全なアクセスの確認が含まれます。
