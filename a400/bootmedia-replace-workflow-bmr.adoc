...
---
= 自動ブートリカバリプロセスを使用する AFF A400 システムのブートメディア交換ワークフロー
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ブートイメージの自動リカバリでは、システムが適切なブートメニューオプションを自動的に識別して選択します。パートナー ノードのブート イメージを使用して、 AFF A400ストレージ システムの交換用ブート メディアにONTAPを再インストールします。
