...
---
= 詳細な手順書を使用してAFF A400システムをインストールする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A400ストレージシステムのインストールは、サイトの準備からONTAPクラスタの起動まで、これらの詳細なステップバイステップの手順に従って行います。
...
NOTE: カード上のポートラベルが見えない場合は、カードの取り付け方向を確認してください。PCIeコネクタソケットは、A400 および FAS8300/8700 のカードスロットの左側にあります。次に、部品番号でカードを検索してください。 https://hwu.netapp.com["NetApp Hardware Universe の略"^]ポートラベルが表示されたベゼルのグラフィックが確認できます。カードの部品番号は、 `sysconfig -a`コマンドまたはシステム梱包リストで確認できます。
...
NS224シェルフまたはSASシェルフのどちらも、システムにケーブルで接続できます。
