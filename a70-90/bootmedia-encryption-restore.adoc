...
summary: AFF A70またはAFF A90システムの交換用ブートメディアに暗号化をリストアして、データを継続的に保護します。このプロセスでは、キーの可用性の確認、暗号化設定の再適用、データへのセキュアなアクセスの確認が行われます。 
---
= ブートメディアでの暗号化のリストア（AFF A70およびAFF A90）
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
