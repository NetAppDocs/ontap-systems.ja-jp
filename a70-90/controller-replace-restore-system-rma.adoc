...
---
= AFF A70またはAFF A90システムのコントローラーの交換を完了する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A70 または AFF A90 システムのコントローラ交換を完了するには、まず NetApp Storage Encryption の設定を復元します（必要な場合）。次に、論理インターフェイス（LIF）がホームポートにレポートしていることを確認し、クラスタの健全性チェックを実行します。最後に、故障した部品を NetApp に返送してください。
