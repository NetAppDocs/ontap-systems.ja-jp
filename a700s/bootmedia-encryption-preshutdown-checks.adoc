...
summary: AFF A700sシステム上の障害のあるコントローラをシャットダウンする前に、暗号化キーのサポートとステータスを確認してください。AFF A700sシステムは、手動のブート メディア リカバリ手順のみをサポートします。自動ブート メディア リカバリはサポートされていません。 
---
= AFF A700sシステムのブートメディアの暗号化サポートを確認してください
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A700sシステム上の障害のあるコントローラをシャットダウンする前に、暗号化キーのサポートとステータスを確認してください。この手順には、 ONTAPバージョンとNetApp Volume Encryption (NVE) の互換性の確認、キー マネージャ構成の検証、ブート メディア リカバリ中のデータ セキュリティを確保するための暗号化情報のバックアップが含まれます。
