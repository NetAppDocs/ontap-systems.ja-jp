...
---
= AFF A800システムのケーブルを再配線し、ディスクを再割り当てする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
コントローラの交換後に AFF A800 システムのケーブルを再接続し、ディスクを交換後のコントローラに再割り当てします。システムが HA ペア構成の場合、ギブバックプロセスによってシステム ID がディスクに自動的に割り当てられます。具体的な手順は、システムが暗号化を使用していないか、Onboard Key Manager （OKM）による暗号化を使用しているかによって異なります。
...
ストレージシステムがHAペア構成の場合、手順の最後にギブバックが発生すると、新しいコントローラモジュールのシステムIDがディスクに自動的に割り当てられます。交換用コントローラを起動する際には、システムIDの変更を確認し、システムがその変更を適用したことを確認する必要があります。

この手順は、HAペアでONTAPを実行しているシステムにのみ適用されます。
