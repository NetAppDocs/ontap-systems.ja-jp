...
summary: AFF A800 システムを完全な動作状態に戻すには、必要に応じて NetApp Storage Encryption 構成をリストアし、新しいコントローラのライセンスをインストールして、障害が発生した部品を NetApp に返却します。 
---
= AFF A800システムのコントローラーの交換を完了する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A800システムのコントローラー交換を完了するには、NetAppストレージ暗号化構成を復元し（必要な場合）、論理インターフェイス（LIF）がホームポートにレポートしていることを確認し、クラスタの健全性チェックを実行して、障害が発生した部品をNetAppに返却します。
