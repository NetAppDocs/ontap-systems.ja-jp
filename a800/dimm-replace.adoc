...
summary: システムが修正可能なエラー訂正コードまたは修正不可能なECCエラーの増加を検知した場合は、AFF A800コントローラモジュールのDIMMを交換してください。故障したDIMMを交換しないと、システムパニックを引き起こす可能性があります。 
---
= AFF A800システムのメモリエラーを修正するためにDIMMを交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ストレージシステムが過剰な訂正可能（CECC）または訂正不可能なメモリエラーを検出した場合は、AFF A800システムのDIMMを交換してください。このようなエラーにより、ストレージシステムがONTAPを起動できなくなる場合があります。

.作業を開始する前に
* システム内の他のすべてのコンポーネントが正常に機能していることを確認してください。正常に動作しない場合は、テクニカルサポートにお問い合わせください。
