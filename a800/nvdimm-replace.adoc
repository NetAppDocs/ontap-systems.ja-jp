...
summary: システムでフラッシュの寿命がほぼ終了したこと、または特定されたNVDIMMが正常でないことが登録された場合は、AFF A800コントローラモジュールのNVDIMMを交換してください。障害が発生したNVDIMMを交換しないと、システムパニックが発生する可能性があります。 
---
= AFF A800システムのNVDIMMを交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
システムがフラッシュメモリの寿命がほぼ尽きている、または識別されたNVDIMMが正常でないと登録した場合は、AFF A800システムのNVDIMMを交換してください。故障したNVDIMMを交換しないと、システムがパニック状態に陥ります。

.作業を開始する前に
* 交換用のNVDIMMが NetApp から届いており、お客様の AFF A800 システムと互換性があることを確認してください。
* ストレージシステムの他のすべてのコンポーネントが正常に機能していることを確認してください。そうでない場合は、NetApp サポートにお問い合わせください。
