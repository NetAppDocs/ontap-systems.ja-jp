...
summary: AFF A900 システムを完全動作状態に戻すには、ストレージを再配線し、必要に応じて NetApp Storage Encryption の設定をリストアし、新しいコントローラーのライセンスをインストールします。 
---
= AFF A900 システムのコントローラー交換を完了する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A900システムのコントローラー交換を完了するには、NetAppストレージ暗号化構成を復元し（必要な場合）、論理インターフェイス（LIF）がホームポートにレポートしていることを確認し、クラスタの健全性チェックを実行して、障害が発生した部品をNetAppに返却します。
