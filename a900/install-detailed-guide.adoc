...
---
= 詳細な手順書を使用してAFF A900システムをインストールする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
以下の詳細な手順に従って、サイトの準備から ONTAP クラスタの起動まで、AFF A900 ストレージシステムをインストールしてください。
...
+
NS224シェルフは、シェルフID 00と01にプリセットされています。シェルフIDを変更する場合は、ボタンが配置されている穴に挿入するツールを作成する必要があります。詳細な手順については、link:../ns224/change-shelf-id.html["シェルフ ID - NS224 シェルフを変更します"]を参照してください。
