...
.手順
. 交換用コントローラがメンテナンス・モード（「 *>」 プロンプトが表示されている）の場合は、メンテナンス・モードを終了して、 LOADER プロンプト「 halt 」に進みます
. 交換用コントローラの LOADER プロンプトからコントローラをブートし、システム ID の不一致によりシステム ID を上書きするように求められた場合は、「 y 」と入力します。
. 交換用コントローラのコンソールに Waiting for giveback... メッセージが表示されるまで待ち、正常なコントローラから新しいパートナーシステム ID が自動的に割り当てられていることを確認します。 storage failover show
+
...
+
交換用コントローラに属するディスクには、新しいシステム ID が表示されます。次の例では、 node1 で所有されているディスクに、新しいシステム ID 1873775277 が表示されています。
