...
. プロンプトでクラスタのオンボードキーマネージャのパスフレーズを入力します。Need example here ******
. バックアップキーのデータを入力します
+

NOTE: パスフレーズの情報が完了すると、ノードがブートメニューに戻ります。
...
. プロンプトに管理証明書の情報を入力します。Need example here *****
+

NOTE: 管理証明書情報が完了すると、ノードがブートメニューに戻ります。
...
. ディスクのMSIDをリセットします。
+
.. オンボードストレージ暗号化を使用している場合は、の手順 に従います https://docs.netapp.com/us-en/ontap/encryption-at-rest/assign-authentication-keys-seds-onboard-task.html["FIPS ドライブまたは SED にデータ認証キーを割り当てる（オンボードキー管理）"^]
.. 外部キー管理ツールを使用しています。の手順 に従います https://docs.netapp.com/us-en/ontap/encryption-at-rest/assign-authentication-keys-seds-external-task.html["FIPS ドライブまたは SED にデータ認証キーを割り当てる（外部キー管理）"^]
