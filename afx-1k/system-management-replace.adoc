...
コントローラの背面、スロット8の左側に位置するシステム管理モジュールには、システム管理用のオンボードコンポーネントと外部管理用のポートが含まれています。故障したシステム管理モジュールを交換する場合、またはブートメディアを交換する場合は、対象コントローラをシャットダウンして電源を切る必要があります。
...
. HAインターコネクトリンクがダウンした場合は、復旧してください：
+
...
** AutoSupportが有効になっていない場合は、 https://mysupport.netapp.com["ネットアップサポート"^]に連絡してシリアル番号を登録してください。
