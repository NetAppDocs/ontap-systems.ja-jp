...
summary: ストレージリソースの制御を交換用コントローラに戻すことで、AFX 2Kストレージシステムが正常に動作を再開できるようになります。 
---
= コントローラーをAFX 2Kストレージシステムに戻して、通常の動作を再開してください。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ストレージリソースの制御を交換用コントローラに戻すことで、AFX 2Kストレージシステムが正常に動作を再開できるようになります。返却手順は、システムで使用されている暗号化の種類（暗号化なし、または Onboard Key Manager （OKM）暗号化）によって異なります。
