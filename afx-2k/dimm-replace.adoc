...
summary: 修正可能なメモリエラーまたは修正不可能なメモリエラーが過剰に検出された場合は、afx 2kストレージシステムのDIMMを交換してください。このようなエラーが発生すると、ストレージシステムが ONTAP を起動できなくなる可能性があります。交換プロセスには、障害のあるコントローラをシャットダウンし、取り外し、DIMMを交換し、コントローラを再取り付けし、故障した部品を NetApp に返却する手順が含まれます。 
---
= DIMMの交換 - AFX 2K
:allow-uri-read: 
:icons: font
