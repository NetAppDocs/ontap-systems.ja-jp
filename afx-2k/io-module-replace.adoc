...
summary: AFX 2KストレージシステムのI/Oモジュールに不具合が生じた場合、またはより高いパフォーマンスや追加機能に対応するためのアップグレードが必要な場合は、I/Oモジュールを交換してください。交換プロセスには、コントローラのシャットダウン、故障したI/Oモジュールの交換、コントローラの再起動、および故障した部品の NetApp への返却が含まれます。 
---
= AFX 2KストレージシステムのI/Oモジュールを交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFX 2KストレージシステムのI/Oモジュールに不具合が生じた場合は、交換してください。交換プロセスには、コントローラのシャットダウン、故障したI/Oモジュールの交換、コントローラの再起動、および故障した部品の NetApp への返却が含まれます。
...
NOTE: 以下の図は、垂直I/Oモジュールを取り外す様子を示しています。通常、取り外すのはI/Oモジュール1つだけです。

+
image:../media/drw_afx_2k_io_remove_replace_ieops-2878.svg["I/Oモジュールの取り外し"]
