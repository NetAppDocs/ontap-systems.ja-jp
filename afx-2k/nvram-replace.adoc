...
summary: AFX 2K ストレージ システムの NVRAM は、不揮発性メモリに不具合が生じた場合、またはアップグレードが必要になった場合に交換してください。 
---
= AFX 2K ストレージシステムの NVRAM を交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFX 2K ストレージ システムの NVRAM は、不揮発性メモリに不具合が生じた場合、またはアップグレードが必要になった場合に交換してください。交換プロセスには、障害が発生したコントローラーのシャットダウン、NVRAM モジュールまたは NVRAM DIMM の交換、および故障した部品の NetApp への返却が含まれます。
...
+
image::../media/drw_afx_2k_nvram_remove_replace_ieops-2881.svg[NVRAM12モジュールとDIMMの取り外し]
...
+
image::../media/drw_afx_2k_nvram_remove_replace_ieops-2881.svg[NVRAM 12モジュールとDIMMの取り外し]
...
NOTE: オーバーライドシステムIDは、NVRAM DIMMの交換ではなく、NVRAMモジュールの交換時にのみ必要です。
...
. コントローラをギブバックします。
+
.. 正常なコントローラから、交換したコントローラのストレージをギブバックします：`_storage failover giveback -ofnode impaired_node_name_`
+
