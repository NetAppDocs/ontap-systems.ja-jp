...
summary: システムが修正可能なエラー訂正コードまたは修正不可能なECCエラーの増加を検知した場合は、ASA C400コントローラモジュールのDIMMを交換してください。故障したDIMMを交換しないと、システムパニックを引き起こす可能性があります。 
---
= ASA C400システムのメモリエラーを修正するためにDIMMを交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ASA C400ストレージシステムで、過剰な訂正可能エラー訂正コード（ECC）エラーまたは訂正不可能なECCエラー（通常は単一のDIMMの障害が原因）が発生した場合は、システム内のDIMMを交換してください。
