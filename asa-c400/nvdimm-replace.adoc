...
summary: システムがフラッシュメモリの寿命がほぼ尽きた、または識別されたNVDIMMが全体的に正常でないと判断した場合、コントローラモジュール内のNVDIMMを交換する必要があります。そうしないと、システムパニックが発生します。 
---
= ASA C400 システムの NVDIMM を交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ストレージシステムが、通常は単一のNVDIMMの障害によって発生する過剰な訂正可能なECCエラーまたは訂正不可能なECCエラーを検出した場合は、ASA C400システムのNVDIMMを交換してください。
