...
summary: 継続的なデータ保護を確保するために、 ASA C800システムの交換用ブート メディアで暗号化を復元します。交換プロセスでは、キーの可用性の確認、暗号化設定の再適用、データへの安全なアクセスの確認が行われます。 
---
= ASA C800 システムで手動ブートリカバリプロセス後にブートメディアの暗号化を復元する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
継続的なデータ保護を確保するために、 ASA C800システムの交換用ブート メディアで暗号化を復元します。交換プロセスでは、キーの可用性の確認、暗号化設定の再適用、データへの安全なアクセスの確認が行われます。
