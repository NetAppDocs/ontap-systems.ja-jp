...
summary: システムがフラッシュメモリの寿命がほぼ尽きた、または識別されたNVDIMMに異常があると判断した場合、コントローラモジュール内のNVDIMMを交換する必要があります。そうしないと、システムパニックが発生します。 
---
= ASA C800 システムの NVDIMM を交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
フラッシュの寿命がほぼ終了したか、識別された NVDIMM が全体的に正常ではないことがシステムによって記録された場合は、 ASA C800システム内の NVDIMM を交換してください。交換しないと、システム パニックが発生します。

.作業を開始する前に
* NetAppから受け取った交換用 NVDIMM がASA C800システムと互換性があることを確認してください。
