...
summary: ストレージ リソースの制御を交換用コントローラに戻すと、ASA C30 ストレージ システムは通常運転を再開できます。ギブバック手順は、システムで使用されている暗号化のタイプ（暗号化なし、Onboard Key Manager 暗号化、External Key Manager 暗号化など）によって異なります。 
---
= ASA C30 システムのコントローラーを返却して通常運用を再開する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ストレージ リソースの制御を交換用コントローラに戻して、ASA C30 ストレージ システムが通常の操作を再開できるようにします。ギブバック手順は、システムで使用されている暗号化タイプ（暗号化なし、オンボードキーマネージャ（OKM）暗号化、外部キーマネージャ（EKM）暗号化）によって異なります。
