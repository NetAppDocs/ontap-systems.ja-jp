...
summary: ASA C30ストレージシステムのコントローラ交換を完了するには、まず必要に応じてNetApp Storage Encryptionの設定をリストアし、新しいコントローラに必要なライセンスをインストールします。次に、論理インターフェイスがホームポートにレポートしていることを確認し、クラスタのヘルスチェックを実行します。最後に、新しいコントローラのシリアル番号を登録してから、障害が発生した部品をNetAppに返却します。 
---
= ASA C30 システムのコントローラー交換を完了する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
