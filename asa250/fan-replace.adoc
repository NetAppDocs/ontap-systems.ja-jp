...
[role="lead"]
ASA A250 システムの故障したファンモジュールを交換するには、障害のあるコントローラーをシャットダウンし、コントローラーモジュールを開けて、交換用ファンを取り付けます。この手順により、コントローラーのコンポーネントが冷却され、熱に関連するシステム障害が防止されます。
