...
summary: 環境変数を確認したら、オンボードキーマネージャ（OKM）、NetApp Storage Encryption（NSE）、またはNetApp Volume Encryption（NVE）が有効になっているシステムに固有の手順を完了する必要があります。 
---
= ASA A400 システムで手動ブートリカバリプロセス後にブートメディアの暗号化を復元する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
