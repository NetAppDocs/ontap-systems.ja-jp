...
summary: ストレージリソースの制御を交換用コントローラに戻すと、ASA A800 システムは通常動作を再開できます。 
---
= ASA A800システムでコントローラーをギブバックして通常運用を再開する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ストレージ リソースの制御を交換用コントローラに戻して、 ASA A800システムが通常の動作を再開できるようにします。返却手順は、システムで使用されている暗号化の種類（暗号化なしまたはオンボード キー マネージャー (OKM) 暗号化）によって異なります。
...
この手順は、HA ペアでONTAP を実行しているシステムにのみ適用されます。
