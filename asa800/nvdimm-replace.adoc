...
summary: ASA A800システムで、フラッシュの寿命がほぼ終了している、または特定されたNVDIMMが全体的に正常でないことがシステムに登録された場合は、NVDIMMを交換してください。交換しないと、システムパニックが発生します。 
---
= ASA A800 システムの NVDIMM を交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
フラッシュの寿命がほぼ終了したか、識別された NVDIMM が全体的に正常ではないことがシステムによって記録された場合は、 ASA A800システム内の NVDIMM を交換してください。交換しないと、システム パニックが発生します。

.作業を開始する前に
* NetAppから受け取った交換用 NVDIMM がASA A800システムと互換性があることを確認してください。
