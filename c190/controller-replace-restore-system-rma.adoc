...
summary: システムを完全に動作可能な状態に復元するには、NetApp Storage Encryptionの設定をリストアし、新しいコントローラのライセンスをインストールし、障害が発生した部品をキットに同梱されているRMA手順書の説明に従ってNetAppに返却する必要があります。 
---
= AFF C190 システムのコントローラー交換を完了する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
NetApp Storage Encryption の設定を復元し（必要な場合）、新しいコントローラのライセンスをインストールし、障害ドライブを NetApp に返却することで、AFF C190 システムのコントローラの交換を完了します。NetApp



[[step-1-installing-licenses-for-the-replacement-controller-in-ontap]]
== 手順1：ONTAP に_replacement _ controllerのライセンスをインストールします
