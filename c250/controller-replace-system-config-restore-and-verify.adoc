...
---
= AFF C250ストレージシステムのコントローラー構成を復元して検証する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ハードウェアの交換が完了し、AFF C250システムをメンテナンス モードで起動したら、交換用コントローラの低レベルのシステム構成を確認し、必要に応じてシステム設定を再構成する必要があります。
...
.次の手順
システム構成を復元して検証した後、link:controller-replace-recable-reassign-disks.html["システムのケーブルを再接続し、ディスクを再割り当てする"]する必要があります。
