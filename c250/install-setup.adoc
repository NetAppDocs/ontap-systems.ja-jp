---
permalink: c250/install-setup.html 
sidebar: sidebar 
keywords: aff c250, install, setup, initial configuration 
summary: AFF C250ストレージシステムのインストールとセットアップを行うには、クイック手順、詳細手順、またはビデオの説明から選択してください。 
---
= AFF C250ストレージシステムのインストール方法を選択してください
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
AFF C250ストレージシステムのインストールとセットアップを行うには、お好みのフォーマットと経験レベルに応じて、クイック手順、詳細手順、またはビデオ説明からお選びください。

* link:../c250/install-quick-guide.html["クイックステップ"]
+
ステップバイステップの手順と追加コンテンツへのライブリンクが記載された PDF 形式のガイドです。

* link:../c250/install-videos.html["ビデオの手順"]
+
手順を追ったビデオでご確認ください。

* link:../c250/install-detailed-guide.html["詳細な手順"]
+
ステップバイステップの手順と追加コンテンツへのライブリンクが記載されたオンライン形式のガイドです。



システムが MetroCluster IP 構成の場合は、を参照してください https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/index.html["MetroCluster IP 構成をインストールします"^] 手順
