...
[role="lead"]
NetAppストレージ暗号化設定をリストア（必要な場合）、交換用コントローラのライセンスをインストールし、論理インターフェイス（LIF）がホームポートに報告されていることを確認します。次に、クラスタヘルスチェックを実行し、交換用コントローラのシリアル番号を登録し、障害が発生したパーツをNetAppに戻して、AFF C30およびC60コントローラの交換手順の最後の手順を完了します。
