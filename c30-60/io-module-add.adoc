...
summary: AFF C30またはAFF C60ストレージシステムにI/Oモジュールを追加して、ネットワーク接続を強化し、システムのデータトラフィック処理能力を拡張します。 
---
= I/O モジュールを AFF C30 または AFF C60 システムに追加する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
I/OモジュールをAFF C30またはAFF C60ストレージシステムに追加することで、ネットワーク接続性を強化し、システムのデータトラフィック処理能力を拡張できます。この手順を使用すると、計画的なアップグレードとして稼働中のコントローラにI/Oモジュールを追加したり、メンテナンス中にI/Oモジュールに不具合のあるコントローラにI/Oモジュールを追加したりできます。

AFF C30およびAFF C60ストレージシステムにI/Oモジュールを追加できるのは、使用可能なスロットがある場合、またはすべてのスロットにフル実装した場合です。

.このタスクについて
この手順は、HAペアのどちらのコントローラに対しても実行できます。コントローラのI/Oモジュールが現在正常に動作しているか、または障害が発生しているかは関係ありません。稼働中のコントローラにはI/Oモジュールを追加できないため、モジュールをインストールする前にターゲットコントローラをシャットダウンする必要があります。HAパートナーは、手順の実行中にストレージ運用を引き継ぎ、クライアントがデータにアクセスできる状態を維持します。

* 対象コントローラが*稼働中*（計画的な追加）の場合は、記載されているシャットダウン手順に従ってください。ただし、その手順では、対象コントローラのI/Oモジュールに障害が発生しているものとして扱ってください。
* 対象コントローラのI/Oモジュールに不具合がある場合は、記載されているシャットダウン手順に従ってください。
...
== 手順1：ターゲットコントローラモジュールをシャットダウンします

ターゲットコントローラをテイクオーバーしてシャットダウンすることで、I/Oモジュールを追加している間もHAパートナーがデータの提供を継続できるようにします。リンク先の手順では、シャットダウンするコントローラが障害のあるI/Oモジュールをホストしていることを前提としています。これらの手順は、計画的な追加のためにコントローラをテイクオーバーする場合でも、既に障害のあるI/Oモジュールを搭載したコントローラをシャットダウンする場合でも適用されます。
...
.作業を開始する前に
*  https://hwu.netapp.com["NetApp Hardware Universe の略"^]を確認して、新しいI/Oモジュールがお客様のストレージシステムおよび実行中のONTAPのバージョンと互換性があることを確認してください。
