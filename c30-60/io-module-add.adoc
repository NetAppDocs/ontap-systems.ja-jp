...
---
= I/OモジュールをAFF C30またはAFF C60ストレージシステムに追加して、ネットワーク接続性を強化します
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF C30またはAFF C60ストレージシステムにI/Oモジュールを追加して、ネットワーク接続性を強化し、システムのデータトラフィック処理能力を拡張します。この手順を使用すると、計画的なアップグレードとして稼働中のコントローラにI/Oモジュールを追加する場合も、メンテナンス中に障害が発生したコントローラにI/Oモジュールを追加する場合も対応できます。

AFF C30およびAFF C60ストレージシステムにI/Oモジュールを追加できるのは、使用可能なスロットがある場合、またはすべてのスロットにフル実装した場合です。

.このタスクについて
この手順は、HAペアのどちらのコントローラに対しても実行できます。コントローラが現在正常に動作しているか、障害が発生しているかは関係ありません。稼働中のコントローラにはI/Oモジュールを追加できないため、モジュールをインストールする前にターゲットコントローラをシャットダウンする必要があります。HAパートナーは、処理中にストレージ運用を引き継ぎ、クライアントがデータにアクセスできる状態を維持します。

* 対象コントローラが*稼働中*（計画的に追加予定）の場合は、記載されているシャットダウン手順に従い、対象コントローラをその内容における「障害のある」コントローラとして扱います。
* ターゲットコントローラに障害が発生している場合は、記載されているシャットダウン手順に従ってください。
...
I/Oモジュールの追加中もHAパートナーがデータを提供し続けるように、ターゲットコントローラモジュールをテイクオーバーしてシャットダウンします。リンク先の手順では、シャットダウンするコントローラを「障害のある」コントローラと呼んでいます。計画的な追加のために稼働中のコントローラをテイクオーバーする場合でも、すでに障害が発生しているコントローラをシャットダウンする場合でも、同じ手順が適用されます。
