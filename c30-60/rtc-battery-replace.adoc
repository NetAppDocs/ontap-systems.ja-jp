...
summary: AFF C30またはAFF C60ストレージシステムのリアルタイムクロックバッテリ（一般的にコイン型電池として知られています）を交換して、正確な時刻同期に依存するサービスやアプリケーションが確実に稼働し続けるようにします。 
---
= AFF C30またはAFF C60ストレージシステムのリアルタイムクロックバッテリーを交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
正確な時刻同期に依存するサービスとアプリケーションが引き続き動作できるように、 AFF C30 またはAFF C60 ストレージ システムのリアルタイム クロック (RTC) バッテリ (一般にコイン型電池と呼ばれる) を交換します。
...
+
 `system location-led _off_`コマンドを入力することでオフにできます。LEDが点灯しているか消灯しているか不明な場合は、 `system location-led show`コマンドを入力して状態を確認してください。
...
|===
. つまみネジを反時計回りに回して、コントローラーのカバーを開けてください。
...
NOTE: RTCバッテリーを交換し、コントローラーを挿入して電源を入れた後、最初のBIOSリセット中に、次のエラーメッセージが表示される場合があります：
`RTC date/time error. Reset date/time to default`
`RTC power failure error`これらのメッセージは想定内のものであり、そのままこの手順を続行していただいて構いません。
