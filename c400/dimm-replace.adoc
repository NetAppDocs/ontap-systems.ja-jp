...
summary: システムで修正可能なエラー訂正コード（ECC）の数が増加している場合は、コントローラモジュール内のDIMMを交換する必要があります。交換しないと、システムパニックが発生します。 
---
= AFF C400ストレージシステムのDIMMを交換してメモリエラーを修正する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF C400ストレージシステムで、過剰な訂正可能なECCエラーまたは訂正不可能なECCエラー（通常は単一のDIMMの障害が原因）が発生した場合は、DIMMを交換してください。
