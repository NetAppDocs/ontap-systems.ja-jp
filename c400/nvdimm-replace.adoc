...
summary: システムがフラッシュの寿命がほぼ終わりに近づいていること、または特定されたNVDIMMが全体的に正常でないことを登録した場合は、コントローラモジュールのNVDIMMを交換する必要があります。交換しないと、システムパニックが発生します。 
---
= AFF C400ストレージシステムのNVDIMMを交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF C400ストレージシステムで過剰な訂正可能なECCエラーまたは訂正不可能なECCエラーが検出された場合は、NVDIMMを交換してください。これは通常、単一のNVDIMMの故障が原因です。
