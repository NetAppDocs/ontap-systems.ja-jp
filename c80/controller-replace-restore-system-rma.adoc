...
summary: 論理インターフェイスがホームポートに報告していることを確認し、クラスタの健全性チェックを実行し、障害が発生したパーツをNetAppに戻して、AFF C80コントローラの交換ワークフローの最後の手順を実行します。 
---
...
[role="lead"]
論理インターフェイスがホームポートに報告していることを確認し、クラスタの健全性チェックを実行し、障害が発生したパーツをNetAppに戻して、AFF C80コントローラの交換ワークフローの最後の手順を実行します。
