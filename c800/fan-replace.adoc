...
[role="lead"]
ファンが故障したり、効率的に動作しなくなった場合は、 AFF C800システムのファン モジュールを交換してください。システムの冷却と全体的なパフォーマンスに影響する可能性があります。交換プロセスには、障害のあるコントローラのシャットダウン、取り外し、ファン モジュールの交換、コントローラの再インストール、そして障害のある部品のNetAppへの返送が含まれます。
...
コントローラモジュールを交換する場合、またはコントローラモジュール内部の部品を交換する場合は、シャーシからコントローラモジュールを取り外してください。
