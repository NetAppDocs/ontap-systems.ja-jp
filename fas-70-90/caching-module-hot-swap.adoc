...
---
= FAS70またはFAS90システムのFlash Cacheモジュールキャリアまたはキャッシングモジュールを交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.作業を開始する前に
* ストレージシステムに、交換用 Flash Cache モジュールキャリアに適したオペレーティングシステムがインストールされていることを確認してください。
* 他のすべてのコンポーネントが正常に機能していることを確認してください。そうでない場合は、テクニカルサポートにお問い合わせください。
...
.作業を開始する前に
* 交換用のキャッシュモジュールが、故障したモジュールと同じ容量であること、同じベンダー製であること、またはサポート対象の別のベンダー製であることを確認してください。
* 他のすべてのコンポーネントが正常に機能していることを確認してください。そうでない場合は、テクニカルサポートにお問い合わせください。
...
.. カムレバーの開口部に指を引っ掛けてモジュールを引き出し、キャッシュモジュールを Flash Cache モジュールキャリアから取り外します。
+
...
+
コマンド出力でステータスが `powered-on`と報告され、運用準備が完了していることを確認してください。
