...
summary: FAS70またはFAS90システムで、修正可能なメモリエラーまたは修正不可能なメモリエラーが過剰に検出された場合は、DIMMを交換してください。このようなエラーにより、ストレージシステムがONTAPを起動できなくなる可能性があります。交換プロセスには、障害のあるコントローラをシャットダウンし、取り外し、DIMMを交換し、コントローラを再取り付けし、故障した部品をNetAppに返却することが含まれます。 
---
= FAS70またはFAS90システムでDIMMを交換してメモリエラーを修正する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS70またはFAS90システムで、修正可能なメモリエラーまたは修正不可能なメモリエラーが過剰に検出された場合は、DIMMを交換してください。このようなエラーにより、ストレージシステムがONTAPを起動できなくなる可能性があります。交換プロセスには、障害のあるコントローラをシャットダウンし、取り外し、DIMMを交換し、コントローラを再取り付けし、故障した部品をNetAppに返却することが含まれます。
...
| FAS70 | スロット3、10、19、26 


| FAS90 | スロット3、7、10、14、19、23、26、30 
|===
