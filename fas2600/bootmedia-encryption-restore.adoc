...
summary: FAS2600システムの交換用ブートメディア上で暗号化構成を復元します。この手順には、Onboard Key Manager、NetApp Storage Encryption、またはNetApp Volume Encryptionが有効になっているシステムの交換後の手順の完了が含まれ、安全なデータアクセスと適切なシステム動作を確保します。 
---
= FAS2600システムのブートメディアの暗号化を復元する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS2600 システムの交換用ブート メディアで暗号化構成を復元します。この手順には、安全なデータ アクセスと適切なシステム操作を確保するために、オンボード キー マネージャー (OKM)、 NetAppストレージ暗号化 (NSE)、またはNetAppボリューム暗号化 (NVE) が有効になっているシステムの交換後の手順を完了することが含まれます。

FAS2600 システムは、手動のブート メディア回復手順のみをサポートします。自動ブート メディア リカバリはサポートされていません。
