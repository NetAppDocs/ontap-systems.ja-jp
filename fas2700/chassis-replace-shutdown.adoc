...
summary: シャーシを交換する前に、お使いの構成に応じた適切な手順に従って、FAS2700システム内の障害コントローラをシャットダウンするか、テイクオーバーしてください。 
---
= シャーシを交換するために FAS2700 コントローラをシャットダウンします。
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
シャーシを交換する前に、お使いの構成に応じた適切な手順に従って、FAS2700システム内の障害コントローラをシャットダウンするか、テイクオーバーしてください。
...
. 前の手順で特定したIPアドレスを使用して、各コントローラのSP/BMCにログインします：
+
** SSH 経由で BMC からログインする場合は、SP/BMC アドレス（例： `ssh admin@<SP/BMC_address>`）を使用して接続し、コマンド `system console`を入力して認証します。
** ローカルコンソールまたはコントローラに直接接続されているラップトップを使用している場合は、同じクラスタ管理者のクレデンシャルを使用してログインします。
