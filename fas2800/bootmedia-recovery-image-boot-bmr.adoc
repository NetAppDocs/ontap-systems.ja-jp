...
summary: FAS2820システムの交換用ブートメディアにONTAPイメージをリストアします。このプロセスでは、LOADERプロンプトからブートリカバリプロセスを開始し、リカバリプロセスが完了してノードが再起動するまで監視します。 
---
= 自動ブートリカバリプロセスを使用して FAS2820 システムでリカバリイメージをブートする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
新しいブートメディアデバイスをFAS2820システムにインストールしたら、自動ブート メディア回復プロセスを開始して、パートナー ノードから構成を復元できます。リカバリプロセス中、システムは暗号化が有効になっているかどうかを確認し、使用されているキー暗号化の種類を特定します。キー暗号化が有効になっている場合、システムはそれを復元するための適切な手順を案内します。
