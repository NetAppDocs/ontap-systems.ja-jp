...
summary: 必要に応じてNetApp Storage EncryptionまたはVolume Encryptionの構成を復元し、交換用コントローラのライセンスをインストールして、キットに同梱されているRMA指示書の説明に従って障害パーツをNetAppに返却することにより、FAS2820システムを完全な動作状態に復旧します。 
---
= FAS2820 システムのコントローラーの交換を完了する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
必要に応じてNetApp Storage EncryptionまたはVolume Encryptionの構成を復元し、交換用コントローラのライセンスをインストールして、キットに同梱されているRMA指示書の説明に従って障害パーツをNetAppに返却することにより、FAS2820システムを完全な動作状態に復旧します。
