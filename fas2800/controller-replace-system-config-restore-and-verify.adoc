...
summary: FAS2820システムでハードウェアの交換を完了し、交換用コントローラーを起動したら、交換用コントローラーの低レベルシステム構成を確認し、必要に応じてシステム設定を再構成します。 
---
= FAS2820 システムでコントローラー構成を復元して検証する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS2820システムでハードウェアの交換を完了し、交換用コントローラーを起動したら、交換用コントローラーの低レベルシステム構成を確認し、必要に応じてシステム設定を再構成します。
...
. 表示されたコントローラのシステム状態がシステム構成と一致しない場合は、交換用コントローラモジュールのHA状態を設定します。 `ha-config modify controller _HA-state_`
+
