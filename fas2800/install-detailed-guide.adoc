...
summary: この手順では、FAS2820ストレージシステムのインストールに関する詳細な手順を段階的に説明します。 
---
= 詳細な手順を使用してFAS2820システムをインストールする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
この手順では、FAS2820 ストレージシステムの準備、設置、ケーブル接続、および電源投入に関する詳細な手順を段階的に説明します。簡略化されたクイックガイドよりも詳細な説明をご希望の場合は、以下の手順に従ってください。
...
+
.. 要件と手順を確認してください。
.. セットアップワークシートに記入して、ストレージシステムに関する情報を収集してください。
...
NOTE: システムに外部シェルフがない場合は、link:../sas3/install-cabling-worksheets-examples-fas2600.html#fas2820-platform-in-a-multipath-ha-configuration-with-no-external-shelves["外部シェルフなしのマルチパスHA構成におけるFAS2820のケーブル接続手順"]を参照してください。
...
ネットワーク検出が有効になっている場合はオプション1、有効になっていない場合はオプション2のいずれかを使用して、ストレージシステムのセットアップと構成を完了してください。
...
--
ノートパソコンでネットワーク検出が有効になっている場合は、自動クラスタ検出を使用してストレージシステムのセットアップと構成を完了してください。

.手順
. この手順の冒頭にあるアニメーションを使用して、シェルフの電源をオンにし、シェルフIDを設定します。
. コントローラの電源をオンにします
+
...
+
.. 選択 https://mysupport.netapp.com["mysupport.netapp.com"]
.. アカウントを作成する必要がある場合、またはアカウントにログインする場合は、「アカウント作成」を選択してください。
...
.. <<手順 1 ：設置の準備>>で収集したデータを使用して、ストレージシステムを設定します。
