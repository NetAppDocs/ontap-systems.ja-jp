...
summary: FAS50ストレージシステムの交換用ブートメディアで暗号化を復元し、データ保護を継続してください。交換プロセスでは、キーの可用性の確認、暗号化設定の再適用、データへの安全なアクセスの確認が行われます。 
---
= 手動ブートリカバリプロセスを使用して、FAS50システムのブートメディアの暗号化を復元する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
