...
summary: FAS50 システムのコントローラー交換を完了するには、まず必要に応じて NetApp Storage Encryption の設定をリストアし、新しいコントローラーに必要なライセンスをインストールします。次に、論理インターフェイスがそれぞれのホームポートにレポートしていることを確認し、クラスタの健全性チェックを実行します。最後に、新しいコントローラのシリアル番号を登録し、故障した部品を NetApp に返送してください。 
---
= FAS50システムのコントローラー交換を完了する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS50システムのコントローラを交換するには、最初にNetAppストレージ暗号化設定をリストアし（必要な場合）、必要なライセンスを新しいコントローラにインストールします。次に、論理インターフェイス（LIF）がホームポートにレポートしていることを確認し、クラスタの健全性チェックを実行します。最後に、新しいコントローラのシリアル番号を登録し、障害が発生したパーツをNetAppに返却します。
