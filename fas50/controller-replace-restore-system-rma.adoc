...
summary: FAS50 システムのコントローラ交換を完了するには、まず必要に応じてNetApp Storage Encryption 構成を復元し、新しいコントローラに必要なライセンスをインストールします。次に、論理インターフェイス LIF がホーム ポートにレポートしていることを確認し、クラスターのヘルス チェックを実行します。最後に、新しいコントローラのシリアル番号を登録し、故障した部品をNetAppに返送します。 
---
= コントローラ交換後の処理- FAS50
:allow-uri-read: 
:icons: font
