...
summary: FAS50 ストレージ システムのコントローラーを交換するには、次のワークフロー手順に従います。 
---
= FAS50コントローラー交換ワークフロー
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS50 ストレージ システムのコントローラーを交換するには、次のワークフロー手順に従います。
...
[role="quick-margin-para"]
コントローラの交換には、故障したコントローラの取り外し、FRU（交換可能ユニット）コンポーネントの交換用コントローラへの移動、交換用コントローラの設置、時刻と日付の設定、および配線の再接続が含まれます。
