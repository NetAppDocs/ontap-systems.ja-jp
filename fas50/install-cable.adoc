...
summary: FAS50ストレージシステムハードウェアを設置したら、コントローラをネットワークおよびシェルフにケーブル接続します。 
---
= FAS50ハードウェアをネットワークおよびストレージ接続用にケーブル接続します
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image::../media/drw_isi_fas50_4p_25gbe_optional_cabling_ieops-1934.svg[FAS50を10/25GbEイーサネットホストネットワークスイッチにケーブル接続]
...
+
その他のサポートされているSASシェルフのケーブル配線も同様です。link:../sas3/install-new-system.html["新しいシステムを設置する場合のシェルフの設置とケーブル接続"^]を参照してください。また、link:../sas3/overview-cabling-rules-examples.html["SASケーブル配線規則、ワークシート、および例の概要"^]も参照できます。
