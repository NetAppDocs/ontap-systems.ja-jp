...
summary: 環境変数を確認した後、オンボードキーマネージャ OKM、NetApp Storage Encryption NSE、または NetApp Volume Encryption NVE が有効になっているシステムに固有の手順を完了する必要があります。 
---
= FAS500f システムのブートメディアの暗号化を手動ブートリカバリプロセス後に復元します
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ブートメディアを交換した後、FAS500f システムがオンボードキーマネージャ、NetApp Storage Encryption、または NetApp Volume Encryption を使用してデータを保護している場合は、暗号化を復元してください。
