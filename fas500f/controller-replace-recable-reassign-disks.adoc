...
summary: ストレージリソースの制御を交換用コントローラに戻して、FAS500f システムが通常の動作を再開できるようにします。 
---
= FAS500f システムのコントローラーを返却して通常運用を再開する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ストレージリソースの制御を交換用コントローラに戻すと、FAS500f システムは通常動作を再開できます。システム構成の復元と検証が完了したら、システムのケーブルを再接続し、ディスクを再割り当てしてください。
...
.次の手順
システムのケーブルを繋ぎ直し、ディスクを再割り当てした後は、link:controller-replace-restore-system-rma.html["コントローラの交換後の処理"]。
