...
[role="lead"]
FAS500fシステムのコントローラ交換を完了して完全に動作させるには、NetApp Storage Encryptionの設定をリストア（必要な場合）し、新しいコントローラのライセンスをインストールし、障害が発生した部品をキットに同梱されているRMA手順書の説明に従ってNetAppに返却する必要があります。
