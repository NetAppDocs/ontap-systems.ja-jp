...
summary: FAS500fシステムのコントローラ交換を完了するには、必要に応じてNetApp Storage Encryptionの設定をリストアし、新しいコントローラのライセンスをインストールして、障害が発生した部品をNetAppに返却します。 
---
= FAS500f システムのコントローラー交換を完了する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS500f システムのコントローラーの交換を完了するには、NetApp ストレージ暗号化の設定を復元し（必要な場合）、新しいコントローラーのライセンスをインストールし、キットに同梱されている RMA 手順書に記載されているとおりに障害ドライブの部品を NetApp に返却してください。
