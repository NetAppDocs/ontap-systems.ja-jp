...
summary: 2 ノードMetroCluster構成でブート メディアの交換を完了した後、 FAS8200システムでMetroClusterスイッチバック操作を実行します。FAS8200システムは、手動のブート メディア回復手順のみをサポートします。自動ブート メディア リカバリはサポートされていません。 
---
= 2 ノード MetroCluster 構成 - FAS8200 のアグリゲートをスイッチバックします
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
2 ノードMetroCluster構成でブート メディアの交換を完了した後、 FAS8200システムでMetroClusterスイッチバック操作を実行します。この手順には、ノードとストレージ仮想マシン (SVM) の状態の確認、スイッチバック コマンドの実行、同期元 SVM がローカル ディスク プールからデータを提供する通常の動作状態に構成が戻ることの確認が含まれます。
