...
summary: システムがモジュールがオフラインになったという単一のAutoSupportメッセージを登録した場合は、コントローラモジュールのキャッシングモジュールを交換する必要があります。交換しないと、パフォーマンスが低下します。 
---
= FAS8200 システムのキャッシュモジュールを交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* ストレージシステムのその他のコンポーネントはすべて正常に機能している必要があります。正常に機能していない場合は、テクニカルサポートにお問い合わせください。
+
