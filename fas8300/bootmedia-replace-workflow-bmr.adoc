...
---
= ブートメディアの自動リカバリワークフロー - FAS8300 および FAS8700
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ブートイメージの自動リカバリでは、システムが適切なブートメニューオプションを自動的に識別して選択します。パートナー ノードのブート イメージを使用して、 FAS8300またはFAS8700ストレージ システムの交換用ブート メディアにONTAPを再インストールします。
