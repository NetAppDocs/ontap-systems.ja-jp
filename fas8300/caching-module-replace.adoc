...
summary: システムがモジュールがオフラインになったという単一のAutoSupportメッセージを登録した場合は、コントローラモジュールのキャッシングモジュールを交換する必要があります。 
---
= FAS8300またはFAS8700システムのキャッシュモジュールを交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: VER2コントローラモジュールには、FAS8300のキャッシュモジュールソケットが1つしかありません。FAS8700にはVER2コントローラモジュールが搭載されていません。ソケットを取り外しても、キャッシュモジュールの機能には影響しません。
...
NOTE: 実行し `system controller flash-cache show`Flash CacheのデバイスIDがわからない場合は、コマンドをます。
...
NOTE: スロット6はFAS8300 VER2コントローラーでのみ使用できます。

状況に応じて、ストレージシステムが次に示す特定の条件を満たしている必要があります。

* 取り付けるキャッシングモジュールに適したオペレーティングシステムが必要です。
* キャッシュ容量をサポートする必要があります。
* キャッシュモジュールの内容は暗号化されていますが、モジュールを交換する前にその内容を消去することをお勧めします。詳細については、NetApp Support Site でお使いのシステムの https://mysupport.netapp.com/info/web/ECMP1132988.html["ボラティリティの声明"]を参照してください。
+

NOTE: ご使用のシステムに対応したボラティリティの声明を表示するには、ネットアップサポートサイトにログインする必要があります。

* ストレージシステム内の他のすべてのコンポーネントも正常に機能している必要があります。そうでない場合は、テクニカルサポートにお問い合わせください。
...
. コントローラモジュール上のFRUマップを使用して、故障したキャッシュモジュールを特定し、取り外します。
+
構成によっては、コントローラモジュールにキャッシュモジュールが0個、1個、または2個搭載されている場合があります。キャッシングモジュールの位置を確認するには、コントローラモジュール内のFRUマップを参照してください。
