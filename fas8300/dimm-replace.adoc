...
summary: ストレージシステムで、過剰な訂正可能エラー訂正コードや訂正不能エラーなどのエラーが発生した場合、コントローラー内のDIMMを交換する必要があります。これらのエラーは通常、単一のDIMMの故障によって発生し、ストレージシステムがONTAPを起動できなくなります。 
---
= FAS8300またはFAS8700システムのDIMMを交換してメモリエラーを修正する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ストレージシステムで、ヘルスモニターのアラートに基づく過剰なCECC（訂正可能エラー訂正コード）エラーや、訂正不可能なECCエラーなどのエラーが発生した場合は、コントローラー内のDIMMを交換する必要があります。これらのエラーは通常、ストレージシステムがONTAPを起動できなくなる単一のDIMM障害によって引き起こされます。

システム内の他のすべての構成要素も正常に機能していなければなりません。コンポーネントが正常に機能していない場合は、テクニカルサポートにお問い合わせください。
...
システムのDIMMの数と位置は、システムのモデルによって異なります。詳細については、空調ダクトに貼付されているFRUマップを参照してください。

* FAS8300 システムを使用している場合、システム DIMM はソケット 2 、 4 、 13 、 15 にあります。
* FAS8700 システムを使用している場合、システム DIMM はスロット 2 、 4 、 7 、 9 、 13 にあります。 15 、 18 、 20 。
* NVDIMM はスロット 11 にあります。
...
+
DIMMはスロットにぴったりと収まりますが、簡単に挿入できるはずです。それでもうまくいかない場合は、DIMMをスロットに合わせて位置を調整し、再度挿入してください。
...
NOTE: 残りのケーブルは、この手順の後半でコントローラーモジュールに接続します。

. コントローラモジュールの取り付けを完了します。
+
.. ロックラッチが持ち上がり始めるまで、ロックラッチを使用してコントローラモジュールをシャーシにしっかりと押し込みます。
+
