...
summary: ストレージシステムで、過剰な訂正可能ECCメモリエラーや訂正不能ECCエラーなどのエラーが発生した場合は、コントローラ内のDIMMを交換する必要があります。これらのエラーは通常、単一のDIMMの故障によって発生し、ストレージシステムによるONTAPの起動を妨げます。 
---
...
[role="lead"]
ストレージシステムで、ヘルスモニターのアラートに基づく過剰な訂正可能ECCメモリエラーや、訂正不可能なECCエラーなどのエラーが発生した場合は、コントローラー内のDIMMを交換する必要があります。これらのエラーは通常、ストレージシステムがONTAPを起動できなくなる単一のDIMM障害によって引き起こされます。
