...
次のアニメーション、図面、または記載された手順に従って、コントローラモジュールをシャーシから取り外すことができます。
...
[role="lead"]
システム DIMM を交換するには、関連するエラーメッセージで対象の DIMM を特定し、エアダクトの FRU マップまたはマザーボードの点灯している LED から対象の DIMM の場所を確認し、 DIMM を交換します。

次に示すアニメーション、図面、または記載された手順を使用して、システム DIMM を交換できます。


NOTE: このアニメーションでは、 DIMM のないソケットには空きスロットが示されています。これらの空のソケットには何も挿入されていません。

https://netapp.hosted.panopto.com/Panopto/Pages/embed.aspx?id=175c6597-449e-4dc8-8c02-aad9012f08b5["システム DIMM の交換"]

image::../media/drw_A400_Replace-NVDIMM-DIMM.png[DRW A400 NVDIMM DIMM の交換]

システムの DIMM の数と位置は、システムのモデルによって異なります。詳細については、エアダクトの FRU マップを参照してください。
...
** テストでエラーが報告されなかった場合は、メニューから [Reboot] を選択してシステムを再起動します。
