...
---
= 詳細な手順書を使用してFAS8300またはFAS8700システムをインストールする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
この詳細なステップバイステップの手順書では、FAS8300またはFAS8700システムのラックへの設置やケーブル接続から初期セットアップまで、インストール全体をご案内します。クイック手順よりも詳細なインストール手順が必要な場合は、以下の手順に従ってください。
...
image:../media/oie_cable_fiber_lc_connector.png["LCファイバコネクタ"]
 a| 
メザニンカード用の16Gb FCまたは25GbEケーブル（注文内容により異なります）
...
NOTE: カードのポートラベルが見えない場合は、カードの取り付け方向を確認し（A400 および FAS8300/8700 では、PCIe コネクタソケットはカードスロットの左側にあります）、部品番号でカードを https://hwu.netapp.com["NetApp Hardware Universe の略"^]で検索して、ポートラベルが表示されたベゼルのグラフィックを確認してください。カードの部品番号は、 `sysconfig -a`コマンドまたはシステム梱包リストで確認できます。
...
NOTE: この例では、XXXXXは対象ノードのシステムシリアル番号を表します。
