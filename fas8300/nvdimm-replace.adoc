...
summary: システムがフラッシュメモリの寿命がほぼ尽きた、または識別された NVDIMM が全体的に正常でないと判断した場合、コントローラモジュール内の NVDIMM を交換する必要があります。 
---
= FAS8300またはFAS8700システムのNVDIMMを交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NVDIMMを交換するには、スロット1ライザーの上部にあるFRUマップを使用して、コントローラモジュール内のNVDIMMの位置を特定する必要があります。

* システムを停止すると、コンテンツのデステージ中に NVDIMM の LED が点滅します。デステージが完了すると LED は消灯します。
* NVDIMMの内容は暗号化されていますが、交換する前にNVDIMMの内容を消去することをお勧めします。詳細については、NetApp Support Siteの https://mysupport.netapp.com/info/web/ECMP1132988.html["ボラティリティの声明"]を参照してください。
+
...
+
NVDIMMはスロットにぴったりと収まりますが、簡単に挿入できるはずです。うまくいかない場合は、NVDIMMをスロットに合わせて位置を調整し、再度挿入してください。
...
. ストレージをギブバックして、コントローラを通常の動作に戻します。 storage failover giveback -ofnode `_impaired_node_name _`
