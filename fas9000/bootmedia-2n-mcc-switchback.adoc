...
summary: '2 ノードMetroCluster構成で FRU の交換が完了したら、 MetroClusterスイッチバック操作を実行できます。これにより、構成は通常の動作状態に戻り、以前障害が発生していたサイトの同期元のストレージ仮想マシン (SVM) がアクティブになり、ローカル ディスク プールからデータを提供するようになります。FAS9000システムは、手動のブート メディア回復手順のみをサポートします。' 
---
