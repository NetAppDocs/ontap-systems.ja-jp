...
summary: 2ノードのMetroCluster構成でFRU交換が完了したら、MetroClusterスイッチバック処理を実行できます。これにより、構成が通常の動作状態に戻り、以前障害が発生していたサイト上の同期ソースStorage Virtual Machine（SVM）がアクティブになり、ローカルディスクプールからデータを提供するようになります。FAS9000システムは、手動によるブートメディアリカバリ手順のみをサポートしています。 
---
= FAS9000システムの2ノードクラスタMetroCluster構成でアグリゲートをスイッチバックする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ブートメディアの交換が完了したら、FAS9000システムでMetroClusterスイッチバック操作を実行します。この手順により、構成が通常の動作状態に復元され、以前障害が発生していたサイト上の同期ソースストレージ仮想マシン（SVM）がアクティブになり、ローカルディスクプールからデータを提供するようになります。FAS9000システムは、手動によるブートメディア復旧手順のみをサポートしています。
