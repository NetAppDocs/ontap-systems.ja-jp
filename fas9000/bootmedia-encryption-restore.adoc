...
summary: 環境変数を確認したら、Onboard Key Manager（OKM）、NetApp Storage Encryption（NSE）、またはNetApp Volume Encryption（NVE）が有効になっているシステムに固有の手順を完了する必要があります。FAS9000システムは、手動によるブートメディアリカバリ手順のみをサポートしています。 
---
= FAS9000システムのブートメディアの暗号化を復元します
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS9000 システムの交換用ブートメディアの暗号化を復元するには、Onboard Key Manager （OKM）など、お使いのキーマネージャーの種類に応じた手順を完了してください。FAS9000 システムは、手動によるブートメディア復旧手順のみをサポートしています。
