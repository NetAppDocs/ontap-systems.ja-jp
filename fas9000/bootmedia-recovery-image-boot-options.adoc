...
summary: 障害のあるノードをリカバリ イメージから起動する手順は、システムが 2 ノードのMetroCluster構成になっているかどうかによって異なります。FAS9000システムは、手動のブート メディア回復手順のみをサポートします。 
---
= FAS9000 システムでリカバリイメージをブートする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
交換用ブートメディアをFAS9000システムにインストールした後、USBドライブからリカバリイメージを起動します。障害が発生したコントローラの起動手順は、システムが2ノードMetroCluster構成かどうかによって異なります。FAS9000システムは、手動によるブートメディア復旧手順のみをサポートしています。
