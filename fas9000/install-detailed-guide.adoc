...
---
= 詳細な手順を使用して FAS9000 ストレージシステムをインストールする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
この詳細なステップバイステップの手順書では、ラックへの設置やケーブル接続から初期セットアップまで、FAS9000システムのインストール全体をご案内します。クイック手順よりも詳細なインストール手順が必要な場合は、以下の手順に従ってください。
...
新しいシステムをDS212C、DS224C、またはNS224シェルフにケーブル接続できます。これは、AFFシステムかFASシステムかによって異なります。
...
ONTAP 9.8以降を実行しているAFF A700またはASA AFF A700システムで、各コントローラをNS224ドライブシェルフのNSMモジュールにケーブル接続する必要があります。

* このタスクは、ONTAP 9.8以降を実行する 環境 ASA AFF A700のみです。
* システムには、各コントローラにつき、スロット3または7に少なくとも1つのX91148Aモジュールが取り付けられている必要があります。このアニメーションまたはイラストは、このモジュールがスロット3と7の両方に取り付けられた状態を示しています。
...
+
システムにNS224ドライブシェルフが搭載されている場合、シェルフはシェルフID 00と01にプリセットされています。シェルフIDを変更する場合は、ボタンが配置されている穴に挿入するツールを作成する必要があります。
...
+
.. エクスプローラを開きます。
.. 左側のペインで「ネットワーク」を選択し、「ネットワーク」を右クリックして、「更新」を選択します。
.. いずれかの ONTAP アイコンをダブルクリックし、画面に表示された証明書を受け入れます。
+

NOTE: 表示されている値は、対象ノードのシステムシリアル番号です。
