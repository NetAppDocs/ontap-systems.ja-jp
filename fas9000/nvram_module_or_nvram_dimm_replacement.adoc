...
---
= NVRAM モジュール / NVRAM DIMM （ AFF A700 / FAS9000 ）を交換してください
:icons: font
:imagesdir: ../media/
...
[role="lead"]
設定に応じて、新しいコントローラモジュールへのディスクの再割り当てを確認するか、ディスクを手動で再割り当てする必要があります。HA ペアのストレージシステムの場合、手順の最後でギブバックが実行されると、新しいコントローラモジュールのシステム ID がディスクに自動的に割り当てられます。スタンドアロンシステムでは、 ID をディスクに手動で再割り当てする必要があります。

構成に適した手順を使用する必要があります。

[cols="1,2"]
|===
| コントローラモジュールの冗長性 | 使用する手順 
...
=== 2 ノード MetroCluster 構成のシステムに、システム ID を手動で再割り当てします
