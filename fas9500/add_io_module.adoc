...
[role="lead"]
I/OモジュールをFAS9500システムに追加するには、既に部品が実装されているシステムでNICまたはストレージアダプタを新しいものと交換するか、空いているシャーシスロットに新しいNICまたはストレージアダプタを追加します。
...
NOTE: 熱による問題を防ぐため、使用していないI/Oスロットには必ずブランクを取り付けてください。
...
. ネットワークにスロット3または7を使用する場合は、 `storage port modify -node __<node name>__ -port __<port name>__ -mode network`コマンドを使用して、スロットをネットワーク用途に変換してください。
...
NICからNIC（同じ数のポート）
 a| 
コントローラモジュールがシャットダウンされると、LIFは自動的に移行します。
