...
summary: 環境変数を確認した後、Onboard Key Manager （OKM）、NetApp Storage Encryption （NSE）、またはNetApp Volume Encryption （NVE）が有効になっているシステムに固有の手順を完了する必要があります。 
---
= 手動ブートリカバリプロセス後に FAS9500 システムのブートメディアの暗号化を復元します
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS9500 システムのブートメディアを交換したら、Onboard Key Manager （OKM）などのキーマネージャータイプに応じた手順を実行して、暗号化を復元してください。
