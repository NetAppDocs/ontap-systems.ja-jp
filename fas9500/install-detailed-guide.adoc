...
sidebar: sidebar 
keywords: FAS9500 
summary: このページでは、FAS9500システムのインストール手順を詳しく説明します。 
---
= 詳細な手順を使用して FAS9500 ストレージシステムをインストールする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
このページでは、FAS9500システムのインストール手順を詳細に説明しています。より詳細なインストール手順が必要な場合は、こちらの説明書をご参照ください。
...
X66033A (112-00438) 、 3m
 a| 
image::../media/oie_cable_mini_sas_hd_to_mini_sas_hd.png[Mini SASコネクタ]
 a| 
ストレージ
...
ホスト側のbポート（スロットA4とB4（e4b）、およびスロットA8とB8（e8b））をホストスイッチにケーブルで接続します。
...
** 電源ケーブルを電源ユニットに接続し、それぞれを異なる電源（図示せず）に接続します。PSU1とPSU3はA面のすべてのコンポーネントに電力を供給し、PSU2とPSU4はB面のすべてのコンポーネントに電力を供給します。
...
* ケーブルコネクタの引き抜きタブの正しい向きについては、図の矢印を必ず確認してください。ストレージモジュールのケーブル引き抜きタブは上向きで、シェルフの引き抜きタブは下向きです。
...
.手順
. 以下のアニメーションまたは図を参照して、コントローラを3台のDS224Cドライブシェルフ（ドライブシェルフ1台のスタック1つと、ドライブシェルフ2台のスタック1つ）にケーブル接続してください。
+
...
+
image::../media/drw_a900_power-on_IEOPS-941.svg[システムの電源を入れる]
