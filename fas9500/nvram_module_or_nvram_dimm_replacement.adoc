...
.手順
. 交換用ノードがメンテナンス・モード（「*>`プロンプト」が表示されている）の場合は、メンテナンス・モードを終了して、LOADERプロンプト「halt」に進みます
. 交換用ノードの LOADER プロンプトからノードをブートし、システム ID が一致しないためにシステム ID を上書きするかどうかを尋ねられたら、「 y 」と入力します。
. 交換用ノードのコンソールに「Waiting for giveback...」というメッセージが表示されるまで待ち、正常なノードから、新しいパートナーシステムIDが自動的に割り当てられていることを確認します。「storage failover show
+
...
.. savecoreコマンドが完了するのを待ってからギブバックを実行します
+
次のコマンドを入力すると'savecoreコマンドの進行状況を監視できますsystem node run -node local-node-name partner savecore -s
