...
[role="lead"]
FAS9500システムのI/Oモジュールを交換するには、障害が発生したノードをシャットダウンし、I/Oモジュールを取り外して取り付け直し、その後ノードを再起動する必要があります。

* この手順は、システムでサポートされるすべてのバージョンの ONTAP で使用できます。
* システム内の他のすべてのコンポーネントが正常に機能している必要があります。正常に機能していない場合は、テクニカルサポートにお問い合わせください。
