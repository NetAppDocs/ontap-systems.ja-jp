...
summary: RoCE対応I/Oモジュールをインストールし、ポートを設定し、既存のシェルフのケーブルを再配線し、自動ドライブ割り当てを無効にすることで、NS224シェルフをホットアドするためのAFF A30、AFF C30、AFF A50、またはAFF C60システムを準備します。 
---
= AFF A30、AFF C30、AFF A50、または AFF C60 システムに NS224 シェルフをホットアドする準備をしてください
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A30、AFF C30、AFF A50、またはAFF C60システムにNS224シェルフをホットアドするための準備として、RoCE対応のストレージポートが十分にあることを確認し、それらをストレージ用に設定し、スロットの冗長性を確保するために既存のシェルフの配線を変更し、自動ドライブ割り当てを無効にします。
...
.次の手順
link:hot-add-aff-install.html["シェルフを取り付ける"]に進みます。
